...
export class psychDSFileDeno {
    ...
    constructor(datasetPath, path, ignore) {
        ...
        this.#ignore = ignore;
        try {
            this.#fileInfo = dntShim.Deno.statSync(this._getPath());
        }
        catch (error) {
            if (error.code === 'ENOENT') {
                this.#fileInfo = dntShim.Deno.lstatSync(this._getPath());
            }
        }
    }
    _getPath() {
    ...
    async readBytes(size, offset = 0) {
        ...
        await handle.seek(offset, dntShim.Deno.SeekMode.Start);
    ...
    #openHandle() {
        ...
        return dntShim.Deno.openSync(this._getPath(), openOptions);
    }
}
/* recursive function for readFileTree, crawls through dataset */
export async function _readFileTree(rootPath, relativePath, ignore, parent, context) {
    await requestReadPermission();
    const name = basename(relativePath);
    const tree = new FileTree(relativePath, name, parent);
    if (!parent) {
        for await (const dirEntry of dntShim.Deno.readDir(join(rootPath, relativePath))) {
            if (dirEntry.isFile && dirEntry.name === "dataset_description.json") {
                const file = new psychDSFileDeno(rootPath, join(relativePath, dirEntry.name), ignore);
    ...
    for await (const dirEntry of dntShim.Deno.readDir(join(rootPath, relativePath))) {
        ...
        if (dirEntry.isDirectory) {
            const dirTree = await _readFileTree(rootPath, join(relativePath, dirEntry.name), ignore, tree, context);
 ...
 */
export function readFileTree(rootPath) {
