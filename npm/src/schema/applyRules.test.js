    ...
    fn: async (t) => {
        await t.step('Columns Found', async () => {
            ...
            if (ddFile) {
                ...
                dsContext = new psychDSContextDataset({ datasetPath: invPATH }, ddFile, description);
            }
            ...
            await applyRules(schema, context);
        ...
        await t.step('Fields found', async () => {
            ...
            if (ddFile) {
                const description = ddFile.expanded;
                dsContext = new psychDSContextDataset({ datasetPath: PATH }, ddFile, description);
            }
            const fileName = '/data/raw_data/study-bfi_data.csv';
            const file = new psychDSFileDeno(PATH, fileName, ignore);
            const issues = new DatasetIssues(schema);
        ...
        await t.step('Fields missing', async () => {
            const ddFile = invFileTree.files.find((file) => file.name === 'dataset_description.json');
            let dsContext;
            if (ddFile) {
                const description = ddFile.expanded;
                dsContext = new psychDSContextDataset({ datasetPath: invPATH }, ddFile, description);
        ...
        await t.step('Context missing', async () => {
            const ddFile = noCtxFileTree.files.find((file) => file.name === 'dataset_description.json');
            let dsContext;
            if (ddFile) {
                const description = ddFile.expanded;
                dsContext = new psychDSContextDataset({ datasetPath: noCtxPATH }, ddFile, description);
            }
            const fileName = '/data/raw_data/study-bfi_data.csv';
        ...
        await t.step('@type missing', async () => {
            const ddFile = noTypeFileTree.files.find((file) => file.name === 'dataset_description.json');
            let dsContext;
            if (ddFile) {
                const description = ddFile.expanded;
                dsContext = new psychDSContextDataset({ datasetPath: noTypePATH }, ddFile, description);
            }
            const fileName = '/data/raw_data/study-bfi_data.csv';
        ...
        await t.step('@type incorrect', async () => {
            const ddFile = wrongTypeFileTree.files.find((file) => file.name === 'dataset_description.json');
            let dsContext;
            if (ddFile) {
                const description = ddFile.expanded;
                dsContext = new psychDSContextDataset({ datasetPath: wrongTypePATH }, ddFile, description);
            }
            const fileName = '/data/raw_data/study-bfi_data.csv';
        ...
        await t.step('correct sidecar identified', async () => {
            ...
            if (context.issues.has('INVALID_SCHEMAORG_PROPERTY'))
                assertEquals(context.issues.get('INVALID_SCHEMAORG_PROPERTY').files.has('/data/raw_data/study-bfi_data.json'), true);
        });
    }
});
