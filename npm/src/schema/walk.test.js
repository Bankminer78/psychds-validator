...
import { assert, assertEquals } from '../deps/asserts.js';
import { psychDSContext } from './context.js';
import { walkFileTree } from './walk.js';
...
dntShim.Deno.test('file tree walking', async (t) => {
    await t.step('visits each file and creates a BIDSContext', async () => {
        ...
        for await (const context of walkFileTree(fileTree, issues)) {
            assert(context instanceof psychDSContext, 'walk file tree did not return a BIDSContext');
        }
    });
