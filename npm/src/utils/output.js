 ...
 */
import { prettyBytes } from '../deps/prettyBytes.js';
import { Table } from '../deps/cliffy.js';
import { colors } from '../deps/fmt.js';
/**
 * Format for Unix consoles
 *
 * Returns the full output string with newlines
 */
export function consoleFormat(result, options) {
    const output = [];
...
function formatIssue(issue, options) {
    ...
    const output = [];
    output.push('\t' +
        colors[color](`[${severity.toUpperCase()}] ${issue.reason} (${issue.key})`));
    output.push('');
    let fileOutCount = 0;
...
function formatSummary(summary) {
    ...
    output.push('');
    //Neurostars message
    output.push(colors.cyan('\tIf you have any questions, please post on https://neurostars.org/tags/bids.'));
    return output.join('\n');
}
