...
import { parseStack } from './logger.js';
dntShim.Deno.test('logger', async (t) => {
...
`;
        assertEquals(parseStack(stack), `file:///bids-validator/src/schema/context.ts:170:16`);
    });
...
`;
        assertEquals(parseStack(stack), 'loadHeader (file:///bids-validator/bids-validator/src/files/nifti.ts:18:12)');
    });
});
