...
import { assertEquals, assertObjectMatch } from '../deps/asserts.js';
import { DatasetIssues } from './datasetIssues.js';
dntShim.Deno.test('DatasetIssues management class', async (t) => {
    ...
    await t.step('issues formatted matching the expected IssueOutput type', () => {
        const issues = new DatasetIssues();
        issues.add({ key: 'TEST_ERROR', reason: 'Test issue' });
        assertEquals(issues.hasIssue({ key: 'TEST_ERROR' }), true);
