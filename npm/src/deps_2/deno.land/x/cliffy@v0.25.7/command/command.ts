...
> {
  ...
  protected async executeExecutable(args: string[]) {
    ...
    try {
      const process: dntShim.Deno.Process = dntShim.Deno.run({
        cmd: [command, ...args],
      });
      const status: dntShim.Deno.ProcessStatus = await process.status();
